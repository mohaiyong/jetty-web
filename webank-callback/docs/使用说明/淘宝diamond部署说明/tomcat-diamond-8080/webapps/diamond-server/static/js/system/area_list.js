...
$(function() {
	...
	var delMenu = function(){
		...
		$.messager.confirm("删除菜单", "你确定要删除该菜单信息吗??", function (r) {  
	        if (r) { 
	    		...
	    		var url = '../../systemMenu.do?method=delSystemAreaMenu';
	    		$.ajax({
	    			url:url,
	    			traditional:true,
	    			type:'get',
	...
	var openMenuItem = function(){
		...
		var sysCode = $("#systemSelect").combobox('getValue');
		var rowIndex = $(this).parents('tr.datagrid-row').attr('datagrid-row-index');
//		if( $('#system-list-content p.selected').length == 0){
//			$.messager.alert('提示', '请选择要关闭的县域!', 'info');
//			return false;
//		}
//		var areaCode = $('#system-list-content p.selected').attr('areaCode');
		var areaCode = 'all';
		$.messager.confirm("开启菜单", "你确定要开启该菜单信息吗??", function (r) {  
	        if (r) { 
	    		...
	    		dataModel.areaCode = areaCode;
	    		var url = '../../systemMenu.do?method=colOrOpenAreaMenuItem';
	    		$.ajax({
	    			url:url,
	    			traditional:true,
	    			type:'get',
...
//		var areaCode = $('#system-list-content p.selected').attr('areaCode');
	...
	var editSystemDialog = function(menuId){
		...
		$(html).dialog({  
		    ...
		    buttons: [{
				...
				iconCls:'icon-ok',
				handler:function(){
					var url = "../../systemMenu.do?method=editAreaMenu";
					editAreaMenu(url);
				}
			},{
				text:'取消',
	...
	var showAddChilDialog = function(){
		...
		$(html).dialog({  
		    ...
		    buttons: [{
				...
				iconCls:'icon-ok',
				handler:function(){
					var url = "../../systemMenu.do?method=addAreaMenu";
					addMenu(url);
				}
			},{
				text:'取消',
	...
	{     
	    return ($('#system-menu-list-content').width() - 5) * percent ;      
	}  
	
	function orderinit(url){
		var columns  = [[
		                 {field:'id',title:'菜单名称',hidden:true,align:'center'},
		                 {field:'parentMenuId',title:'parentMenuId',hidden:true,align:'center'},
...
//		});
	...
	var selectedSystem = function(){
		$('#system-list-content p').removeClass('selected');
		$(this).addClass('selected');
		doSearch();
	}
	
	var loadSystemList = function(){
		$.getJSON('../../system.do?method=systemAreaList',function(json){
			var arr = [];
	...
	var doSearch = function (){
		var sysCode = $("#systemSelect").combobox('getValue');
		var areaCode = $('#system-list-content p.selected').attr('areaCode');
		orderinit("../../systemMenu.do?method=systemAreaMenuList&sysCode="+sysCode+"&areaCode="+areaCode);
	}
	
	var initUI = function(){
		$('#form').delegate('#addSystem','click',showSystemDialog);
		$('#system-list-content').delegate('p','click',selectedSystem);
		$("#systemSelect").combobox({
 			onChange:function(){
 				doSearch();
 			}
	...
	var settingUI = function(){
		...
		$('#system-menu-list-content').width(bodyWidth-310);
		$('#system-menu-list-content').height(bodyHeight);
		$('#system-list-content').height(bodyHeight);
