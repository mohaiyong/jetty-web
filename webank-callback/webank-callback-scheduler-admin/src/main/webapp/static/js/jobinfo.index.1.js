$(function() {
	...
	var jobTable = $("#job_list").dataTable({
	    ...
	    "columns": [
					...
					},
					{
						...
						"visible" : true,
						"render": function ( data, type, row ) {
							var controlFlag = (row.controlFlag== 0)?'调度中心控制': '外系统控制';
							return controlFlag;
						}
					},
	                	...
	                	"render": function ( data, type, row ) {
	                		return function(){
                				...
                				if ('NORMAL' == row.jobStatus) {
	                				pause_resume = '<button class="btn btn-primary btn-xs job_operate" ' + ((row.controlFlag == 0)?'':'disabled="disabled"')+ ' _type="job_pause" type="button">暂停</button>  ';
								} else if ('PAUSED' == row.jobStatus){
									pause_resume = '<button class="btn btn-primary btn-xs job_operate" ' +((row.controlFlag == 0)?'':'disabled="disabled"')+ ' _type="job_resume" type="button">恢复</button>  ';
								}
	                			// log url
								...
								var html = '<p id="'+ row.id +'" >'+
									...
									codeBtn +
									'<button class="btn btn-danger btn-xs job_operate" ' + ((row.controlFlag == 0)?'':'disabled="disabled"')+ ' _type="job_del" type="button">删除</button>  '+
									'</p>';
	...
	$("#job_list").on('click', '.update',function() {
        ...
        //如果是外系统控制禁掉属性修改
        if(row.controlFlag == 1){
        	//$("#updateModal .form input[name='jobDesc']").attr("disabled","disabled");
        	//$("#updateModal .form input[name='jobCron']").attr("disabled","disabled");
        	//$("#updateModal .form input[name='executorParam']").attr("disabled","disabled");
        	//$("#updateModal .form input[name='author']").attr("disabled","disabled");
        }
